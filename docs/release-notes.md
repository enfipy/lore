...
- `lore auth info --with-access-token` prints the repository-scoped authorization (access) token for the current user. This is the output-side sibling of the global `--identity-token` / `--access-token` input flags. The `auth info` flag `--with-token` is renamed `--with-identity-token` to tell the two tokens apart, with the old spelling kept as an alias
...
- Fix revisions committed with `lore commit --offline` being unreadable from every other clone once they were pushed. A `lore sync` joins that line in as the second parent of the merge revision it builds, which is not on the branch history a push walks, so nothing uploaded what those revisions own: another clone reported a revision's signature and parent and no branch, date or message, and `lore revision metadata get` answered `Address not found`. An online commit uploads what it writes as it writes it, which is why only the offline case reached this. The push now walks the second parent's line and takes its revisions through the same collection as the ones on the branch, oldest first, uploading what each owns without offering it to the peer as a new latest revision. The walk stops at the peer's latest for the branch the line belongs to, so a line the peer carries already costs nothing beyond naming it, and falls back on the merge's own first parent for a branch the peer has never seen, which stops at the branch point. A revision on that line that is itself a merge is followed the same way. The server collects a merge revision against both of its parents rather than only the first, so a peer that pushes a merge without the tip of the line it merged is refused rather than accepted silently
- `lore revision info` and `lore history` warn when a revision names metadata they cannot read, and report the revision without the branch, date and message that blob carries, rather than reporting it with those fields empty and no indication that anything was missing (`revision info`) or failing outright (`history`). A revision list leaves the states of a segment in the local store without the metadata they name, so a revision read out of that cache offline has nothing to take those fields from, which is ordinary rather than damage
- Fix `lore revision metadata set --binary` leaving its payload on the machine that set it, where every other clone resolved the recorded address to `Address not found`. Push collects the fragments an address-typed revision metadata value names, as it already did for file metadata, and the server verifies them with the revision's other new fragments
